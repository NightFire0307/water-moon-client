import { LeftOutlined, MessageOutlined, RightOutlined, ZoomInOutlined, ZoomOutOutlined } from '@ant-design/icons'
import { Divider, Image, Space, Tooltip } from 'antd'

import { useState } from 'react'
import { TagAddIcon } from '../assets/svg/CustomIcon.tsx'
...
export function PhotoGrid() {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 ">
      <Image.PreviewGroup preview={{ toolbarRender: (_, { transform: { scale }, actions: { onActive, onZoomIn, onZoomOut } }) => (
        <div className="bg-gray bg-opacity-10 pr-4 pl-4 pt-2 pb-2 rounded-[100px] text-gray-500">
          <Space size={14} className="toolbar-wrapper text-xl">
            <Tooltip title="添加备注">
              <MessageOutlined className="hover:text-white cursor-pointer" />
            </Tooltip>
            <Tooltip title="添加标记">
              <TagAddIcon className="hover:text-white cursor-pointer" />
            </Tooltip>
